...
def import_data(sub_index, datatype):
    ...
    if datatype=='T':
        aug_num=10#3
        eeg_parymid=data_parymid(data,loop=aug_num)
        label=np.concatenate([label]*aug_num)
    else:
        eeg_parymid=data_parymid(data,loop=1)
    # label.append(label_one_subject)

    return eeg_parymid, label  # (288, 22, 1000) (288,)
...
def data_parymid(data,expasion=1.8,channel_num=3,loop=3):
    result=[]
    for t in range(loop):
        step=expasion/channel_num
        length=data.shape[3]
        parymid_list=[signal.resample(data,num=length+int(length*step*i),axis=3) for i in range(channel_num)]
        parymid=[parymid_list[0]]
        for i in range(1,channel_num):
            tmp_start=random.randint(0,parymid_list[i].shape[3]-length)
            parymid.append(parymid_list[i][:,:,:,tmp_start:tmp_start+length])
        result.append(np.concatenate(parymid,axis=1))
